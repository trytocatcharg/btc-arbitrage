...
export interface BotConfig {
  ...
  openTrade: {
    ...
    variationalMakerFeeBps: string;
    variationalTakerFeeBps: string;
  };
  risex: {
    apiBaseUrl: string;
...
export function loadBotConfig(env: NodeJS.ProcessEnv = process.env): BotConfig {
  ...
  if (exchangeA === exchangeB)
    throw new Error("EXCHANGE_A and EXCHANGE_B must be different");

  const priceSource = parsePriceSource(env.PRICE_SOURCE ?? "mark");
  const marketType = parseMarketType(env.MARKET_TYPE ?? "perpetual");
  const minPriceDiffUsd = parsePositiveDecimalString(
    env.MIN_PRICE_DIFF_USD ?? "40",
    "MIN_PRICE_DIFF_USD",
  );
  const leverage = parsePositiveInteger(env.LEVERAGE ?? "3", "LEVERAGE");

  const executionMode = parseExecutionMode(
    env.BOT_EXECUTION_MODE ?? ExecutionMode.DryRun,
  );
  const botRunOnce = parseBoolean(env.BOT_RUN_ONCE ?? "false");
  const enableOrderPlacement = parseBoolean(
    env.ENABLE_ORDER_PLACEMENT ?? "false",
  );
  const risexTradingEnabled = parseBoolean(
    env.RISEX_TRADING_ENABLED ?? "false",
  );
  ...
  if (executionMode === ExecutionMode.Live && !enableOrderPlacement) {
    throw new Error(
      "ENABLE_ORDER_PLACEMENT must be true when BOT_EXECUTION_MODE=live",
    );
  }
  if (enableOrderPlacement && executionMode !== ExecutionMode.Live) {
    throw new Error(
      "BOT_EXECUTION_MODE must be live when ENABLE_ORDER_PLACEMENT=true",
    );
  }
  const telegramEnabled = parseBoolean(env.TELEGRAM_ENABLED ?? "false");
  const telegramAlertCooldownMs = parseNonNegativeInteger(
    env.TELEGRAM_ALERT_COOLDOWN_MS ?? "3600000",
  ...
  const openTradeTakeProfitPercent = parsePositiveDecimalString(
    env.OPEN_TRADE_TAKE_PROFIT_PERCENT ?? "3",
  ...
  return {
    ...
    exchangeA,
    exchangeB,
    exchangeLong: env.EXCHANGE_LONG
      ? parseExchange(env.EXCHANGE_LONG, "EXCHANGE_LONG")
      : undefined,
    exchangeShort: env.EXCHANGE_SHORT
      ? parseExchange(env.EXCHANGE_SHORT, "EXCHANGE_SHORT")
      : undefined,
    btcSymbol: env.BTC_SYMBOL ?? "BTCUSDT",
    marketType,
    priceSource,
    pricePollIntervalMs: parsePositiveInteger(
      env.PRICE_POLL_INTERVAL_MS ?? "1000",
    ...
    openTrade: {
      ...
      quoteMaxAgeMs: parsePositiveInteger(
        env.OPEN_TRADE_QUOTE_MAX_AGE_MS ?? "5000",
        "OPEN_TRADE_QUOTE_MAX_AGE_MS",
      ),
      ...
      entryImproveTicks: parseNonNegativeInteger(
        env.OPEN_TRADE_ENTRY_IMPROVE_TICKS ?? "1",
        "OPEN_TRADE_ENTRY_IMPROVE_TICKS",
      ),
      residualDeltaToleranceBase: parsePositiveDecimalString(
        env.OPEN_TRADE_RESIDUAL_DELTA_BTC ?? "0.00001",
        "OPEN_TRADE_RESIDUAL_DELTA_BTC",
      ),
      ...
      risexTakerFeeBps: parseNonNegativeDecimalString(
        env.RISEX_TAKER_FEE_BPS ?? "3",
        "RISEX_TAKER_FEE_BPS",
      ),
      extendedMakerFeeBps: parseNonNegativeDecimalString(
        env.EXTENDED_MAKER_FEE_BPS ?? "0",
        "EXTENDED_MAKER_FEE_BPS",
      ),
      extendedTakerFeeBps: parseNonNegativeDecimalString(
        env.EXTENDED_TAKER_FEE_BPS ?? "2.5",
        "EXTENDED_TAKER_FEE_BPS",
      ),
      variationalMakerFeeBps: parseNonNegativeDecimalString(
        env.VARIATIONAL_MAKER_FEE_BPS ?? "0",
        "VARIATIONAL_MAKER_FEE_BPS",
      ),
      variationalTakerFeeBps: parseNonNegativeDecimalString(
        env.VARIATIONAL_TAKER_FEE_BPS ?? "0",
        "VARIATIONAL_TAKER_FEE_BPS",
      ),
    },
    risex: {
      apiBaseUrl: trimTrailingSlash(
        env.RISEX_API_BASE_URL ?? "https://api.rise.trade",
      ),
      ...
      sessionSignerPrivateKey: emptyToUndefined(
        env.RISEX_SESSION_SIGNER_PRIVATE_KEY,
      ),
      tradingEnabled: risexTradingEnabled,
    },
    extended: {
      apiBaseUrl: trimTrailingSlash(
        env.EXTENDED_API_BASE_URL ?? "https://api.starknet.extended.exchange",
      ),
    ...
    variational: {
      apiBaseUrl: trimTrailingSlash(
        env.VARIATIONAL_API_BASE_URL ?? "https://omni.variational.io/api",
      ),
      priceApiBaseUrl: trimTrailingSlash(
        env.VARIATIONAL_PRICE_API_BASE_URL ??
          "https://omni-client-api.prod.ap-northeast-1.variational.io",
      ),
      accountAddress: emptyToUndefined(env.VARIATIONAL_ACCOUNT_ADDRESS),
      accountPrivateKey: emptyToUndefined(env.VARIATIONAL_ACCOUNT_PRIVATE_KEY),
      referralCode: emptyToUndefined(env.VARIATIONAL_REFERRAL_CODE),
      tradingEnabled: variationalTradingEnabled,
      userAgent: env.VARIATIONAL_USER_AGENT ?? "btc-arbitrage-bot/0.1",
      priceCacheTtlMs: parsePositiveInteger(
        env.VARIATIONAL_PRICE_CACHE_TTL_MS ?? "5000",
        "VARIATIONAL_PRICE_CACHE_TTL_MS",
      ),
    },
    telegram: {
...
export function redactSecrets(value: unknown): RedactedValue {
  ...
  if (typeof value === "object") {
    ...
    for (const [key, entry] of Object.entries(value)) {
      out[key] =
        /token|key|secret|private|signature|authorization|password|databaseUrl|DATABASE_URL/i.test(
          key,
        )
          ? "[REDACTED]"
          : redactSecrets(entry);
...
function buildDatabaseUrl(input: Omit<DatabaseConfig, "url">): string {
  ...
  try {
    ...
    throw new Error(
      `Invalid database configuration: ${error instanceof Error ? error.message : "unknown error"}`,
    );
  }
  return url.toString();
}
...
function parseExchange(value: string, field: string): ExchangeId {
  if (
    ...
    return value;
  throw new Error(
    `${field} must be one of: risex, extended, arcus, variational`,
  );
}
...
function parseExecutionMode(value: string): ExecutionMode {
  if (value === ExecutionMode.DryRun || value === ExecutionMode.Live)
    return value;
