...
export const activeTradeStatuses: readonly TradeStatus[] = [
  "executing_limit",
  "hedging",
  "protecting",
  "open",
...
export const priceSnapshots = mysqlTable(
  "price_snapshots",
  {
  ...
  (table) => ({
    exchangeSymbolIdx: index("price_exchange_symbol_idx").on(
      table.exchangeId,
      table.symbol,
    ),
  }),
...
export const spreadSnapshots = mysqlTable(
  "spread_snapshots",
  {
    id: int("id").autoincrement().primaryKey(),
    symbol: varchar("symbol", { length: 32 }).notNull(),
    ...
    exchangeAPriceUsd: decimal("exchange_a_price_usd", {
      precision: 24,
      scale: 8,
    }).notNull(),
    exchangeBPriceUsd: decimal("exchange_b_price_usd", {
      precision: 24,
      scale: 8,
    }).notNull(),
    absoluteDiffUsd: decimal("absolute_diff_usd", {
      precision: 24,
      scale: 8,
    }).notNull(),
    diffBps: decimal("diff_bps", { precision: 24, scale: 8 }).notNull(),
    direction: varchar("direction", { length: 32 }).notNull(),
    thresholdUsd: decimal("threshold_usd", {
      precision: 24,
      scale: 8,
    }).notNull(),
  ...
  (table) => ({
    symbolCalculatedAtIdx: index("spread_symbol_calculated_at_idx").on(
      table.symbol,
      table.calculatedAt,
    ),
    matchedCalculatedAtIdx: index("spread_matched_calculated_at_idx").on(
      table.thresholdMatched,
      table.calculatedAt,
    ),
  }),
);
...
export const signals = mysqlTable(
  "signals",
  {
    ...
    observedDiffUsd: decimal("observed_diff_usd", {
      precision: 24,
      scale: 8,
    }).notNull(),
  ...
  (table) => ({
    statusCreatedAtIdx: index("signal_status_created_at_idx").on(
      table.status,
      table.createdAt,
    ),
...
export const trades = mysqlTable(
  "trades",
  {
    ...
    filledNotionalUsd: decimal("filled_notional_usd", {
      precision: 24,
      scale: 8,
    })
      .notNull()
  ...
  (table) => ({
    statusCreatedAtIdx: index("trade_status_created_at_idx").on(
      table.status,
      table.createdAt,
    ),
    statusOpenedAtIdx: index("trade_status_opened_at_idx").on(
      table.status,
      table.openedAt,
    ),
    signalIdx: index("trade_signal_idx").on(table.signalId),
    symbolStatusIdx: index("trade_symbol_status_idx").on(
      table.symbol,
      table.status,
    ),
  }),
...
export const tradePreviews = mysqlTable(
  "trade_previews",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  (table) => ({
    tokenIdx: index("trade_preview_token_idx").on(table.token),
    signalStatusIdx: index("trade_preview_signal_status_idx").on(
      table.signalId,
      table.status,
    ),
  }),
...
export const tradeLegs = mysqlTable(
  "trade_legs",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  (table) => ({
    ...
    exchangeStatusIdx: index("trade_leg_exchange_status_idx").on(
      table.exchangeId,
      table.status,
    ),
    tradeSideIdx: index("trade_leg_trade_side_idx").on(
      table.tradeId,
      table.side,
    ),
  }),
...
export const tradeStatusHistory = mysqlTable(
  ...
  (table) => ({
    tradeChangedAtIdx: index("trade_status_history_trade_changed_at_idx").on(
      table.tradeId,
      table.changedAt,
    ),
    toStatusChangedAtIdx: index(
      "trade_status_history_to_status_changed_at_idx",
    ).on(table.toStatus, table.changedAt),
  }),
);

export const telegramCommandLogs = mysqlTable(
  ...
  (table) => ({
    chatCreatedAtIdx: index("telegram_command_chat_created_at_idx").on(
      table.chatId,
      table.createdAt,
    ),
    commandCreatedAtIdx: index("telegram_command_command_created_at_idx").on(
      table.command,
      table.createdAt,
    ),
  }),
...
export const operations = mysqlTable(
  ...
  (table) => ({
    ...
    statusCreatedAtIdx: index("operation_status_created_at_idx").on(
      table.status,
      table.createdAt,
    ),
  }),
...
export const events = mysqlTable(
  ...
  (table) => ({
    typeCreatedAtIdx: index("event_type_created_at_idx").on(
      table.type,
      table.createdAt,
    ),
    levelCreatedAtIdx: index("event_level_created_at_idx").on(
      table.level,
      table.createdAt,
    ),
  }),
