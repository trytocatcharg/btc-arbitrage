...
export async function fetchVolumeStats(fetchImpl: typeof fetch = fetch): Promise<VolumeStatsResponse> {
  ...
  if (!response.ok) {
    throw new Error(`Backend volume stats request failed with HTTP ${response.status}`);
  }
