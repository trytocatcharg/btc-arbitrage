 ...
 */
export const FarmedVolumePanel: FC<FarmedVolumePanelProps> = ({ volumeStats }) => {
  ...
  return (
    <section className="rounded-3xl border border-panel-border bg-panel p-6 shadow-2xl shadow-slate-950/50">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-[0.24em] text-slate-500">Volume farming</p>
          <h2 className="mt-2 text-xl font-semibold">Farmed volume</h2>
        </div>
        <div className="flex flex-col items-end gap-1">
          <span className="text-[0.65rem] uppercase tracking-[0.18em] text-slate-500">Farming</span>
          <StatusBadge status={hasVolume ? 'open' : 'planned'} />
        </div>
      </div>

      {hasVolume ? null : (
        <p className="mt-5 text-lg font-semibold text-slate-200">{formatUsd(0)} farmed</p>
      )}

      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Lifetime farmed" value={formatUsd(lifetimeUsd)} tone="positive" emphasis />
        <MetricCard label="Last 24h" value={formatUsd(Number(windows['24h'].totalUsd))} />
        <MetricCard label="Last 7d" value={formatUsd(Number(windows['7d'].totalUsd))} />
        <MetricCard label="Last 30d" value={formatUsd(Number(windows['30d'].totalUsd))} />
      </div>

      <div className="mt-5">
        <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Per-venue breakdown (lifetime)</p>
        <div className="mt-2 grid gap-3 sm:grid-cols-2">
          {lifetime.byVenue.length > 0 ? (
            lifetime.byVenue.map((venue) => (
              <SummaryItem key={venue.exchangeId} label={venue.exchangeId} value={formatUsd(Number(venue.volumeUsd))} />
            ))
          ) : (
      ...
      <p className="mt-4 text-xs text-slate-500">Updated {formatDateTime(generatedAt)}</p>
    </section>
  );
};
