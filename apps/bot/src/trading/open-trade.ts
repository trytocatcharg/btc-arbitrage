  ...
  private async runEntry(
    ...
    try {
      ...
      while (true) {
        ...
        current = await limit.getExecutionOrder(submittedLimit.id);
      }
      if (current.status !== "filled") {
        await limit.cancelExecutionOrder(submittedLimit.id);
      ...
      const limitFillPrice = current.averageFillPriceUsd;
      ...
      if (covered > 0 && (await this.store.claimRollback(token))) {
        ...
        for (const [exchange, side] of [
          ...
          try {
            ...
            outcomes.push(`${exchange} emergency close submitted`);
          } catch {
        ...
        await this.options.notifyUrgent?.(
          `🚨 Trade rollback ${token}: ${outcomes.join("; ")}`,
        );
      }
      await this.store.transition(token, "failed", { error: message });
      throw error;
    }
  }
