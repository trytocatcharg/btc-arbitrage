...
export interface PreviewStore { createPreview(preview: OpenTradePreview): Promise<void>; consumePreview(token: string, now: Date): Promise<OpenTradePreview | null>; startExecution(preview: OpenTradePreview, leverage: number): Promise<void>; claimRollback(token: string): Promise<boolean>; transition(token: string, state: OpenTradeState, details?: Record<string, unknown>): Promise<void>; }
export interface OpenTradeOptions { notionalUsd: string; leverage: number; ttlMs: number; quoteMaxAgeMs: number; limitTimeoutMs: number; residualDeltaToleranceBase: string; takeProfitPercent: string; stopLossPercent: string; fees: Record<ExchangeId, { makerBps: string; takerBps: string }>; notifyUrgent?: (text: string) => Promise<void>; now?: () => Date; sleep?: (ms: number) => Promise<void>; }
...
export class OpenTradeService {
  ...
  async confirm(token: string): Promise<void> {
    ...
    await this.store.startExecution(preview, this.options.leverage); await this.store.transition(token, 'executing_limit');
    ...
    try {
      await Promise.all([limit.validateExecutionPreflight({ symbol: preview.symbol, leverage: this.options.leverage }), market.validateExecutionPreflight({ symbol: preview.symbol, leverage: this.options.leverage }), limit.getAvailableMarginUsd(), market.getAvailableMarginUsd()]);
      ...
      const longEntry = preview.limitExchange === preview.longExchange ? limitFillPrice : marketFillPrice; const longTp = applyPercentChange(longEntry, this.options.takeProfitPercent, 'up'); const longSl = applyPercentChange(longEntry, this.options.stopLossPercent, 'down');
...
function applyPercentChange(value: string, percent: string, direction: 'up' | 'down'): string {
  const multiplier = direction === 'up' ? 1 + (parseDecimal(percent, 'percent') / 100) : 1 - (parseDecimal(percent, 'percent') / 100);
  if (multiplier <= 0) throw new Error('Stop loss percent must keep trigger price above zero');
  return formatDecimal(parseDecimal(value) * multiplier);
}

function selectEntryExecutionExchanges(input: { longExchange: ExchangeId; shortExchange: ExchangeId; fees: OpenTradeOptions['fees'] }): { limitExchange: ExchangeId; marketExchange: ExchangeId } {
  const candidates: Array<{ limitExchange: ExchangeId; marketExchange: ExchangeId; makerBps: number; takerBps: number }> = [
    {
  ...
  return { limitExchange: candidates[0]!.limitExchange, marketExchange: candidates[0]!.marketExchange };
}
