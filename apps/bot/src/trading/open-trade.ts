...
export class OpenTradeService {
  ...
  async confirm(token: string): Promise<void> {
    ...
    try {
      ...
      const submittedLimit = await limit.submitExecutionOrder({ clientOrderId: `${token}-limit`, symbol: preview.symbol, side: limitSide, type: 'limit', quantityBase: preview.quantityBase, priceUsd: preview.limitExchange === preview.shortExchange ? preview.shortPriceUsd : preview.longPriceUsd }); limitOrder = submittedLimit.id;
      ...
      if (current.status !== 'filled') await limit.cancelExecutionOrder(submittedLimit.id);
      if (covered <= 0) { await this.store.transition(token, 'cancelled'); return; }
    ...
    } catch (error) { const message = error instanceof Error ? error.message : String(error); if (covered > 0 && await this.store.claimRollback(token)) { const outcomes: string[] = []; if (limitOrder) { try { await limit.cancelExecutionOrder(limitOrder); outcomes.push('limit cancelled'); } catch { outcomes.push('limit cancel failed'); } } for (const item of protectionOrderIds) { try { await item.adapter.cancelExecutionOrder(item.id); outcomes.push(`protection ${item.id} cancelled`); } catch { outcomes.push(`protection ${item.id} cancel failed`); } } for (const [exchange, side] of [[preview.longExchange, 'sell'], [preview.shortExchange, 'buy']] as const) { try { await this.execution(this.registry.get(exchange)).submitExecutionOrder({ clientOrderId: `${token}-emergency-${exchange}`, symbol: preview.symbol, side, type: 'market', quantityBase: formatDecimal(covered, 10), reduceOnly: true }); outcomes.push(`${exchange} emergency close submitted`); } catch { outcomes.push(`${exchange} emergency close failed`); } } await this.options.notifyUrgent?.(`🚨 Trade rollback ${token}: ${outcomes.join('; ')}`); } await this.store.transition(token, 'failed', { error: message }); throw error; }
  }
