...
import {
  ...
  createExtendedSignedOrder,
  initExtendedSigningWasm,
  roundDecimalToStep,
  type ExtendedFees,
  type ExtendedOrderSide,
  type ExtendedSigningMarket,
  type ExtendedStarknetDomain,
} from "./extended-order-signing.js";
...
export function assertExtendedTradingUnsupported(): never {
  throw new Error(
    "Extended trade execution is intentionally not implemented in this slice. Use dry-run monitoring only.",
  );
}
...
class ExtendedExecutionAdapter implements ExecutionAdapter {
  ...
  async getBestBidOffer(input: PriceRequest): Promise<BestBidOffer> {
    ...
    const orderbook = unwrapData(
      await this.http.get(
        `/api/v1/info/markets/${encodeURIComponent(getMarketName(market))}/orderbook`,
      ),
    );
  ...
  async getMarketMetadata(input: PriceRequest): Promise<MarketMetadata> {
    const market = await this.getMarket(input);
    const tradingConfig = requiredRecord(
      market.tradingConfig,
      "Extended market tradingConfig",
    );
    return {
      ...
      quantityStepBase: decimalField(
        tradingConfig,
        ["minOrderSizeChange"],
        "market.tradingConfig.minOrderSizeChange",
      ),
      priceTickUsd: priceStep(market),
  ...
  async getAvailableMarginUsd(): Promise<string> {
    ...
    const balance = unwrapData(
      await this.http.get("/api/v1/user/balance", { private: true }),
    );
    ...
    return (
      findDecimal(balance, [
        ...
        "balance",
        "equity",
      ]) ?? fail("Extended balance response did not include available margin")
    );
  }
  ...
  async validateExecutionPreflight(input: {
    ...
    if (
      metadata.maxLeverage !== undefined &&
      input.leverage > metadata.maxLeverage
    )
      throw new Error(
        `Extended leverage ${input.leverage} exceeds market max ${metadata.maxLeverage}`,
      );
    if (Number(availableMargin) <= 0)
      throw new Error("Extended available margin is zero");
  ...
  async submitExecutionOrder(
    ...
    this.requireCredentials();
    if (!this.http.post)
      throw new Error("Extended HTTP client does not support POST");
    ...
    const [fees, starknetDomain] = await Promise.all([
      this.getFees(marketName),
      this.getStarknetDomain(),
    ]);
    const ctx = createExtendedOrderContext({
      market: toSigningMarket(market),
      fees,
      starknetDomain,
      vaultId: this.config.vaultId!,
      starkPrivateKey: this.config.starkPrivateKey!,
    });
    const side = toExtendedSide(input.side);
    const quantity = roundDecimalToStep(
      input.quantityBase,
      metadataStep(market),
      "down",
    );
    const order = await this.createOrderPayload(
      input,
      market,
      marketName,
      side,
  ...
  async cancelExecutionOrder(orderId: string): Promise<void> {
    ...
    this.requireApiKey();
    if (!this.http.delete)
      throw new Error("Extended HTTP client does not support DELETE");
    await this.http.delete(
      `/api/v1/user/order/${encodeURIComponent(orderId)}`,
      { private: true },
    );
  }
  ...
  async getPosition(input: {
    ...
    const payload = unwrapData(
      await this.http.get("/api/v1/user/positions", {
        private: true,
        query: { market: getMarketName(market), side },
      }),
    );
    const position = asRecords(payload).find(
      (item) => normalizeSide(item.side) === input.side,
    );
    if (!position) return null;
    const quantityBase =
      findDecimal(position, ["size", "qty", "quantity"]) ?? "0";
    ...
    return {
      ...
      status: "open",
      exitPriceUsd: findDecimal(position, [
        "closePrice",
        "exitPrice",
        "exit_price",
        "close_price",
      ]),
      realizedPnlUsd: findDecimal(position, [
        "realizedPnl",
        "realized_pnl",
        "pnl",
        "realizedPnlUsd",
  ...
  private async createOrderPayload(
    ...
    if (input.type === "limit") {
      ...
      return createExtendedSignedOrder({
        marketName,
        orderType: "LIMIT",
        side,
        amountOfSynthetic: quantity,
        price: roundDecimalToStep(
          input.priceUsd,
          minPriceChange,
          side === "BUY" ? "up" : "down",
        ),
    ...
    if (input.type === "take-profit-market" || input.type === "stop-market") {
      if (!input.triggerPriceUsd)
        throw new Error("Extended TP/SL order requires triggerPriceUsd");
      if (!input.reduceOnly)
        throw new Error("Extended TP/SL execution orders must be reduce-only");
      const triggerPrice = roundDecimalToStep(
        input.triggerPriceUsd,
        minPriceChange,
        side === "BUY" ? "up" : "down",
      );
      const executionPrice = roundDecimalToStep(
        applyBps(
          input.triggerPriceUsd,
          side === "BUY"
      ...
      const trigger = {
        triggerPrice,
        triggerPriceType: "LAST" as const,
  ...
  private async crossingPrice(
    ...
    const orderbook = unwrapData(
      await this.http.get(
        `/api/v1/info/markets/${encodeURIComponent(marketName)}/orderbook`,
      ),
    );
    const best =
      side === "BUY"
        ? firstPrice(orderbook, "ask")
        : firstPrice(orderbook, "bid");
  ...
  private requireTradingEnabled(): void {
    if (!this.config.tradingEnabled)
      throw new Error(
        "Extended trading is disabled; set EXTENDED_TRADING_ENABLED=true only after credentials and risk controls are ready",
      );
  }

  private requireCredentials(): void {
    this.requireApiKey();
    if (!this.config.starkPrivateKey)
      throw new Error(
        "EXTENDED_STARK_PRIVATE_KEY is required for Extended live execution",
      );
    if (!this.config.vaultId)
      throw new Error(
        "EXTENDED_VAULT_ID is required for Extended live execution",
      );
  }
...
function toSigningMarket(
  market: Record<string, unknown>,
): ExtendedSigningMarket {
  ...
  return {
    ...
    l2Config: {
      collateralId: stringField(
        l2Config,
        ["collateralId"],
        "l2Config.collateralId",
      ),
      collateralResolution: decimalField(
        l2Config,
        ["collateralResolution"],
        "l2Config.collateralResolution",
      ),
      syntheticId: stringField(
        l2Config,
        ["syntheticId"],
        "l2Config.syntheticId",
      ),
      syntheticResolution: decimalField(
        l2Config,
        ["syntheticResolution"],
    ...
    tradingConfig: {
      minOrderSizeChange: decimalField(
        tradingConfig,
        ["minOrderSizeChange"],
        "tradingConfig.minOrderSizeChange",
      ),
      maxPositionValue: decimalField(
        tradingConfig,
        ["maxPositionValue"],
...
function firstPrice(payload: unknown, side: "bid" | "ask"): string {
  ...
  if (!Array.isArray(levels) || levels.length === 0)
    throw new Error(`Extended orderbook ${side} side is empty`);
  return decimalField(
    requiredRecord(levels[0], `Extended ${side} level`),
    ["price"],
    `${side}.price`,
  );
}
...
function metadataStep(market: Record<string, unknown>): string {
  return decimalField(
    requiredRecord(market.tradingConfig, "Extended market tradingConfig"),
    ["minOrderSizeChange"],
    "market.tradingConfig.minOrderSizeChange",
  );
}
