...
const PLACE_TPSL_TYPE =
  "PlaceTpslOrder(address account,uint64 marketId,uint8 side,string size,uint8 stopType,string stopPrice,string limitPrice,uint8 orderType,uint8 stopPriceOption,uint8 tif,uint32 deadline)";
const CANCEL_TPSL_TYPE =
  "CancelTpslOrder(address account,string orderId,uint32 deadline)";
...
export function signPlaceTpsl(input: {
  ...
  // RISEx TP/SL endpoints require the 65-byte r||s||v form (verified
  // against the official risex Python SDK); the compact 64-byte form is
  // rejected with "invalid signature length: expected 65 bytes".
  return signDigestToStandardBase64(digest, input.privateKey);
}
...
function createTypedDigest(
  ...
  const domainSeparator = keccakBytes(
    concatBytes(
