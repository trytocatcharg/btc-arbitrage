...
export function abiSignedWord(value: bigint): Uint8Array {
  ...
  if (normalized < 0n || normalized >= max)
    throw new Error("Signed value does not fit int256");
...
export function addressWord(value: string): Uint8Array {
  return concatBytes(
    new Uint8Array(12),
    hexToBytes(normalizeAddress(value).slice(2)),
  );
}
...
export function signDigestToCompactBase64(
  digest: Uint8Array,
  privateKey: string,
): string {
...
export function signDigestToRpcHex(
  digest: Uint8Array,
  privateKey: string,
): string {
  const signature = secp256k1.sign(
...
/** 65-byte `r || s || v` signature (v = 27/28) as base64. RISEx TP/SL
 * endpoints take the signature as a top-level field (not wrapped in a
 * permit) and reject the 64-byte EIP-2098 compact form with "invalid
 * signature length: expected 65 bytes". Verified against the official
 * risex Python SDK (0.10.0), which base64-encodes the standard 65-byte
 * signature for /v1/orders/tpsl. */
export function signDigestToStandardBase64(
  digest: Uint8Array,
  privateKey: string,
): string {
  const signature = secp256k1.sign(
  ...
  const s = bigintToBytes(signature.s, 32);
  const v = Uint8Array.of(signature.recovery + 27);
  return Buffer.from(concatBytes(r, s, v)).toString("base64");
}
