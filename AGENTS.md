...
| `OPEN_TRADE_TAKE_PROFIT_PERCENT` / `OPEN_TRADE_STOP_LOSS_PERCENT` | `3` / `2.5` | TP/SL applied on confirm (per-leg, anchored to each leg's own fill) |
| `OPEN_TRADE_SPREAD_EXIT_TIMEOUT_MINUTES` | `30` | Time-stop: close an open trade after this many minutes regardless of spread |
| `OPEN_TRADE_MIN_PROFIT_USD` | `0.05` | Fill-time edge band: minimum expected convergence beyond round-trip breakeven (fees + slippage) required to keep a trade; below it both legs close immediately (`edge_below_cost`) |
| `OPEN_TRADE_MAX_LOSS_USD` | `0.25` | Runtime assertion bound on the edge-abort path: `\|realizedPnlUsd\|` above this fires an urgent fee-model-drift alert |
| `OPEN_TRADE_SLIPPAGE_BPS` | `2` | Assumed exit slippage (basis points) for the edge band computation |
| `OPEN_TRADE_SPREAD_TP_USD` / `OPEN_TRADE_SPREAD_SL_USD` / `OPEN_TRADE_EDGE_MIN_PROFIT_USD` | — | **Removed** (change `adjust-tpsl-volume-farming`): defining any of them fails config load; spread-USD exits deleted |
...
- Open-trade flow on confirm: create `trades` row (mode `live`) + 2 `trade_legs` (`planned`) → passive limit entry (price = bid/ask, post-only; up to 3 retries only on `PostOnlyOrderMatched`) → poll fill every 250 ms up to `OPEN_TRADE_LIMIT_TIMEOUT_MS`, repricing the resting order at the new best bid/ask every `OPEN_TRADE_REPRICE_INTERVAL_MS` (0 disables) → hedge each fill immediately with a market order on the other exchange → cancel remainder → place TP (`take-profit-market`) and SL (`stop-market`) reduce-only on both legs, anchored to each leg's own fill → status `open`. While the limit rests, each reprice check also runs two guards: the anti-chase guard (never reprice past the point where the executable spread stops covering costs) and the live-spread viability watch (abort the entry if the directional mark spread drops below `MIN_PRICE_DIFF_USD`; wired from config, `spread_dropped` closeReason + Telegram notice). Failure mid-way with covered quantity triggers `claimRollback`: cancel orders + emergency reduce-only market closes + urgent notify + status `failed`. On timeout, the cancel races a possible late fill: the order is re-read with bounded retries (`readOrderAfterCancel`, 4 × 500 ms, max-filled state wins) and a late fill continues the normal hedge/protect flow instead of aborting as `cancelled`.
- Execution setup (RISEx `updateLeverage` write + Extended signing-WASM init) runs **once at bot startup** (`main.ts`), hoisted out of the per-trade path; there is no per-trade preflight or margin read on confirm anymore. Misconfiguration fails fast at boot or surfaces at the submit step.
- **Margin-based TP/SL + disabled exits (since 2026-09-21)**: `OPEN_TRADE_TAKE_PROFIT_PERCENT` / `OPEN_TRADE_STOP_LOSS_PERCENT` are defined **on the margin**, not on price — the price-side trigger distance is the percentage ÷ `LEVERAGE` (at 5x: 3% TP margin = 0.6% price, 2.5% SL margin = 0.5% price). Each leg rests a `take-profit-market` + `stop-market` reduce-only trigger on its own exchange, anchored to its own fill. The fill-time edge band (`EDGE_BAND_ENABLED = false`) and the time-stop close (`BOT_TIME_STOP_ENABLED` env gate) stay disabled. When one leg's TP/SL fills, the sibling stays open and the trade is marked `unhedged` with an urgent notify (position monitor). The stale-`closing` recovery sweep stays active regardless.
